...
public class ExcelGenerator {
    ...
    public void importExcel(MultipartFile file) throws Exception{
       ...
       Sheet sheet1 = workbook.getSheetAt(0);
        ...
        for(int i=0;i<(CoutRowExcel(sheet1.rowIterator()));i++) {
            if(i == 0) {
                continue;
            }
            ...
            String nama = row.getCell(1).getStringCellValue().toString();
            String kelas = row.getCell(2).getStringCellValue().toString();
            String jurusan = row.getCell(3).getStringCellValue().toString();

            siswaDao.save(new Siswa(0, nama, kelas, jurusan));
        }
        Sheet sheet2 = workbook.getSheetAt(1);
        for(int i=0;i<(CoutRowExcel(sheet2.rowIterator()));i++) {
            if(i == 0) {
                continue;
            }
